#!/bin/bash
#SBATCH --job-name=helion-train-droso-v3
...
#SBATCH --output=/storage3/fs1/shandley/Active/helion/logs/%j_train_droso_v3.out
#SBATCH --error=/storage3/fs1/shandley/Active/helion/logs/%j_train_droso_v3.err

# v3 change: dataset normalizes all training examples to sense orientation
# (RC sequence + RC-mapped labels for minus-strand genes), so donor=GT and
# acceptor=AG consistently. Also fixes false donor label on last exon.

set -euo pipefail

HELION_ROOT="/storage3/fs1/shandley/Active/helion"
DATA_DIR="${HELION_ROOT}/data/drosophila"
MODEL_OUT="${HELION_ROOT}/models/drosophila_v3"
